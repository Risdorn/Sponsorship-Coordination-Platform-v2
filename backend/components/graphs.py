...
def ad_request_month_status(sponsor_id):
    campaign_id = [campaign.id for campaign in Campaign.query.filter_by(sponsor_id=sponsor_id).all()]
    ad_requests = Ad_request.query.filter(Ad_request.campaign_id.in_(campaign_id))
    ad_requests = ad_requests.filter(Ad_request.created_on < date.today()).filter(Ad_request.created_on > date.today().replace(month=date.today().month - 1)).all()
    # Get the status of each ad request
    statuses = [ad.status for ad in ad_requests]
    # Make a dictionary
    status_dict = {"Pending": 0, "Accept": 0, "Reject": 0}
...
def payment_distribution_month(sponsor_id):
    campaign_id = [campaign.id for campaign in Campaign.query.filter_by(sponsor_id=sponsor_id).all()]
    ad_requests = Ad_request.query.filter(Ad_request.campaign_id.in_(campaign_id))
    ad_requests = ad_requests.filter(Ad_request.created_on < date.today()).filter(Ad_request.created_on > date.today().replace(month=date.today().month - 1)).all()
    # Get the payment amount of each ad request
